...
- *Implemented* **Create position**  
  Admin goes to `/admin/positions/new`. Primary flow: upload or paste job description → LLM generates 5–10 questions (POST `/api/analyze-jd`) → review → create template + position. Alternative: “Use existing template” or “From scratch” (name, type, optional questions). Position dropdown in header; selecting a position loads its template when `templateId` is set.

- *Implemented* **New position from job description**  
  On `/admin/positions/new`, admin pastes or uploads a job description (.txt). LLM analyzes the JD and generates 5–10 screening questions. Admin reviews, enters position name and type, then creates the position; a new custom template is created with those questions and default screening intro/conclusion/reminder, and the position is linked to that template.
...
- *Implemented* **POST /api/analyze-jd**  
  Request: `{ jobDescription: string }`. Response: `{ questions: Question[] }`. Uses LLM to generate 5–10 screening questions from the JD.
